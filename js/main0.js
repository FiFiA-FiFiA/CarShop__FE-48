...
let select__wrapper = document.querySelectorAll("[select__wrapper]"),
  ...
  yearFrom__options__container = document.querySelector("[yearFrom__options__container]"),
  yearTo__options__container = document.querySelector("[yearTo__options__container]"),
  select__yearFrom__inner = document.querySelector("[select__yearFrom__inner]"),
  select__yearTo__inner = document.querySelector("[select__yearTo__inner]"),
...
function openClose__filter__Dropdown() {
  ...
  document.querySelectorAll('[yearFrom__options__container] .option').forEach(opt => {
    opt.addEventListener('click', () => {
      document.querySelectorAll('[yearFrom__options__container] .option').forEach(o => o.classList.remove('active'))

      opt.classList.add('active');

      YearFromTo__Filter__Value.push(opt.querySelector("label").textContent);
      select__yearFrom__inner.innerText = YearFromTo__Filter__Value[0];
    })
  })

  document.querySelectorAll('[yearTo__options__container] .option').forEach(opt => {
    opt.addEventListener('click', () => {
      document.querySelectorAll('[yearTo__options__container] .option').forEach(o => o.classList.remove('active'))

      opt.classList.add('active');

      YearFromTo__Filter__Value.push(opt.querySelector("label").textContent);
      select__yearTo__inner.innerText = YearFromTo__Filter__Value[1];
    })
  })
  ...
  Filter__Selected__Value__Arr.push(Manufacturer__Filter__Value, Model__Filter__Value, Category__Filter__Value, YearFromTo__Filter__Value);
